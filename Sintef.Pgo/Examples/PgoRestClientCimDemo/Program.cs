...
namespace PgoRestDemo
{
	...
	/// The program loads an example CIM network and forecast, optimizes,
	...
	public class Program
	{
		public static void Main()
		{
			...
			Directory.SetCurrentDirectory("Data/DIGIN10/v2.2");

			new Program().Run(serviceUrl);
		}

		private string _networkFile = "CombinedNetwork.jsonld";
		private string _forecastFile = "CombinedSsh.jsonld";
		...
		void Run(string serviceUrl)
		{
			...
			try
			{
				PgoRestClient client = new PgoRestClient(SessionType.Cim, serviceUrl, null);
				...
				client.LoadNetwork(_networkId, File.OpenRead(_networkFile), _networkFile, new CimParsingOptions { UnitsProfile = CimUnitsProfile.Digin });

				Console.WriteLine();
				Console.WriteLine("*** Setting up a case...");
				client.CreateCimSession(_sessionId, _networkId, _forecastFile);
				...
				var bestSolution = client.GetBestCimSolution(_sessionId);
				Console.WriteLine(bestSolution.PeriodSolutions[0]);

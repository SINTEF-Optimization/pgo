...
In this text we will provide some simpler example models of power grids, to make it easier to see how the software might be used. The examples use the proprietary PGO file format (see [github](https://github.com/SINTEF-Optimization/pgo) for details). The reader is asked to mostly ignore the physical properties, like voltage and generation limits — these cases are made for readability and illustrative purposes. All of these examples can be run with the following simple forecast of 0.5 MW for the single load present:
...
![Line diagram of one provider and one consumer](images/example-provider-consumer-300x219.png)
...
#![Line diagram of two providers and one consumer](images/example-two-providers-one-consumer-300x181.png)
...
An example output from running this case through the service (see [Getting started using the API](api_getting_started.md)) shows that the correct line is opened (i.e. in effect removed from the network):
...
![Line diagram of one transformer and one consumer](images/example-provider-transformer-consumer-300x219.png)
...
![Line diagram with 3-winded transformer](images/3WindedProvider-300x166.png)
